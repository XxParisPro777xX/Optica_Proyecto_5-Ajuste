...
class Aplicacion:
    ...
    def params(self):
        ...
        tk.Button(btn_frame, text="Save CSV", command=self.guardar_csv, width=8).pack(side=tk.LEFT, padx=4)
...
#Extracción de datos para .csv
            self.x_data = np.array(x)    # Distancia [m]
            self.y_data = np.array(y)    # Flujo ideal
            self.x1_data = x1            # Tiempo [s]
            self.y1_data = np.array(y1)  # Flujo muestreado (cámara)
    ...
    def guardar_csv(self):
        # 1. Verificar si ya se calculó la curva
        if not hasattr(self, 'x1_data') or len(self.x1_data) == 0:
            print("No hay datos para guardar. Presiona '1D Calc' primero.")
            return

        # 2. Pedir al usuario dónde guardar el archivo
        archivo = fl.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("Archivos CSV", "*.csv")],
            title="Guardar Perfil Simulado 1D"
        )
        
        if archivo:
            # 3. Matemática Inversa para el Frame: 
            # Tu código original hace: Tiempo = expt * (Frame - cent)
            # Para exportarlo hacemos: Frame = (Tiempo / expt) + cent
            exp = float(self.expt2.get())
            cent = int(self.cent2.get())
            frames = (self.x1_data / exp) + cent

            # 4. Alinear Distancia y Flujo Real (a veces los arreglos x y x1 difieren en longitud por el muestreo)
            # Si no coinciden en longitud, rellenamos con ceros la distancia para evitar errores de guardado.
            distancia = self.x_data if len(self.x_data) == len(self.x1_data) else np.zeros_like(self.x1_data)
            flujo_real = self.y_data if len(self.y_data) == len(self.y1_data) else np.zeros_like(self.y1_data)

            # 5. Estructurar matriz compatible con Tangra / cargar_datos
            # Columna 0: Frame (Leído por cargar_datos)
            # Columna 1: Distancia en metros (Extra para ti en Excel)
            # Columna 2: Flujo Muestreado (Leído por cargar_datos)
            # Columna 3: Tiempo en segundos (Extra para ti en Excel)
            # Columna 4: Flujo Real / Ideal (Extra para ti en Excel)
            datos = np.column_stack((frames, distancia, self.y1_data, self.x1_data, flujo_real))

            # 6. Rellenar las filas de encabezado (skiprows)
            # Como la interfaz omite 14 filas por defecto, creamos 14 líneas falsas
            # para que puedas subir el archivo directamente sin cambiar la cajita de 'skiprows'
            skip = int(self.skiprows2.get())
            encabezado = ["# Metadatos del simulador TAOS II"] * skip
            if skip > 0:
                encabezado[-1] = "Frame,Distancia[m],Flujo_Muestreado,Tiempo[s],Flujo_Real"
            
            # Guardar el CSV
            np.savetxt(archivo, datos, delimiter=',', header='\n'.join(encabezado), comments='')
            print(f"¡Perfil 1D simulado guardado con éxito en: {archivo}!")
